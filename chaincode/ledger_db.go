...
// GetAggregateTraintuple fetches a AggregateTraintuple from the ledger using its unique key
func (db *LedgerDB) GetAggregateTraintuple(key string) (AggregateTraintuple, error) {
...
func (db *LedgerDB) GetOutModelHashDress(traintupleKey string, modelType CompositeModelType, allowedAssetTypes []AssetType) (*HashDress, error) {
	for _, assetType := range allowedAssetTypes {
		...
		case AggregateTraintupleType:
			tuple, err := db.GetAggregateTraintuple(traintupleKey)
			if err == nil {
				return tuple.OutModel, nil
			}
		default:
