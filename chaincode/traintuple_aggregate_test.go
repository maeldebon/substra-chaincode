...
func TestTraintupleWithNoTestDatasetAggregate(t *testing.T) {
	...
	inpTraintuple := inputAggregateTraintuple{ObjectiveKey: objHash}
	args = inpTraintuple.createDefault()
	resp = mockStub.MockInvoke("42", args)

	assert.EqualValues(t, 200, resp.Status, "when adding aggregate traintuple without test dataset it should work: ", resp.Message)

	traintuple := outputAggregateTraintuple{}
	json.Unmarshal(resp.Payload, &traintuple)
	args = [][]byte{[]byte("queryAggregateTraintuple"), keyToJSON(traintuple.Key)}
	resp = mockStub.MockInvoke("42", args)
	assert.EqualValues(t, 200, resp.Status, "It should find the aggregate traintuple without error ", resp.Message)
}
...
func TestTraintupleWithSingleDatasampleAggregate(t *testing.T) {
	...
	inpTraintuple := inputAggregateTraintuple{
		ObjectiveKey: objHash,
		AlgoKey:      aggregateAlgoHash,
	}
	args = inpTraintuple.createDefault()
	resp = mockStub.MockInvoke("42", args)
	assert.EqualValues(t, 200, resp.Status, "when adding aggregate traintuple with a single data samples it should work: ", resp.Message)
...
func TestNoPanicWhileQueryingIncompleteTraintupleAggregate(t *testing.T) {
	...
	registerItem(t, *mockStub, "aggregateTraintuple")
	...
	require.NotPanics(t, func() {
		getOutputAggregateTraintuple(NewLedgerDB(mockStub), traintupleKey)
	})
}
...
func TestTraintupleComputePlanCreationAggregate(t *testing.T) {
	...
	inpTraintuple := inputAggregateTraintuple{ComputePlanID: "someComputePlanID"}
	args := inpTraintuple.createDefault()
	resp := mockStub.MockInvoke("42", args)
	...
	inpTraintuple = inputAggregateTraintuple{Rank: "1"}
	args = inpTraintuple.createDefault()
	resp = mockStub.MockInvoke("42", args)
	...
	inpTraintuple = inputAggregateTraintuple{Rank: "0"}
	args = inpTraintuple.createDefault()
	resp = mockStub.MockInvoke("42", args)
	...
	key := res["key"]
	require.EqualValues(t, aggregateTraintupleKey, key)
	...
	assert.EqualValues(t, aggregateTraintupleKey, errorPayload["key"], "key in error should be aggregateTraintupleKey")
}
...
func TestTraintupleMultipleCommputePlanCreationsAggregate(t *testing.T) {
	...
	args = inpTraintuple.createDefault()
	resp = mockStub.MockInvoke("42", args)
	assert.EqualValues(t, 400, resp.Status, resp.Message, "should failed to add an aggregate traintuple of the same rank")
	...
	args = inpTraintuple.createDefault()
	resp = mockStub.MockInvoke("42", args)
	assert.EqualValues(t, 400, resp.Status, resp.Message, "should failed to add an aggregate traintuple to an unexisting ComputePlanID")
	...
	args = inpTraintuple.createDefault()
	resp = mockStub.MockInvoke("42", args)
	assert.EqualValues(t, 200, resp.Status, resp.Message, "should be able do create an aggregate traintuple with the same ComputePlanID")
	err = json.Unmarshal(resp.Payload, &res)
	assert.NoError(t, err, "should unmarshal without problem")
	assert.Contains(t, res, "key")
	ttkey := res["key"]
...
func TestTraintupleAggregate(t *testing.T) {
	...
	args = inpTraintuple.createDefault()
	resp = mockStub.MockInvoke("42", args)
	assert.EqualValuesf(t, 400, resp.Status, "when adding aggregate traintuple with unexisting algo, status %d and message %s", resp.Status, resp.Message)

	// Properly add traintuple
	resp, tt := registerItem(t, *mockStub, "aggregateTraintuple")

	inpTraintuple = tt.(inputAggregateTraintuple)
	res := map[string]string{}
	err := json.Unmarshal(resp.Payload, &res)
	assert.NoError(t, err, "aggregate traintuple should unmarshal without problem")
	assert.Contains(t, res, "key")
	traintupleKey := res["key"]
	// Query traintuple from key and check the consistency of returned arguments
	args = [][]byte{[]byte("queryAggregateTraintuple"), keyToJSON(traintupleKey)}
	resp = mockStub.MockInvoke("42", args)
	assert.EqualValuesf(t, 200, resp.Status, "when querying the aggregate traintuple - status %d and message %s", resp.Status, resp.Message)
	out := outputAggregateTraintuple{}
	err = json.Unmarshal(resp.Payload, &out)
	assert.NoError(t, err, "when unmarshalling queried aggregate traintuple")
	expected := outputAggregateTraintuple{
		Key: aggregateTraintupleKey,
		Algo: &HashDressName{
	...
	assert.Exactly(t, expected, out, "the aggregate traintuple queried from the ledger differ from expected")

	// Query all traintuples and check consistency
	args = [][]byte{[]byte("queryAggregateTraintuples")}
	resp = mockStub.MockInvoke("42", args)
	assert.EqualValuesf(t, 200, resp.Status, "when querying aggregate traintuples - status %d and message %s", resp.Status, resp.Message)
	// TODO add traintuple key to output struct
	// For now we test it as cleanly as its added to the query response
	assert.Contains(t, string(resp.Payload), "key\":\""+aggregateTraintupleKey)
	var queryTraintuples []outputAggregateTraintuple
	err = json.Unmarshal(resp.Payload, &queryTraintuples)
	assert.NoError(t, err, "aggregate traintuples should unmarshal without problem")
	...
	inpWaitingTraintuple := inputAggregateTraintuple{
		InModels: []string{aggregateTraintupleKey},
	}
	args = inpWaitingTraintuple.createDefault()
	resp = mockStub.MockInvoke("42", args)
	assert.EqualValuesf(t, 200, resp.Status, "when adding aggregate traintuple with status %d and message %s", resp.Status, resp.Message)

	// Query traintuple with status todo and worker as trainworker and check consistency
	filter := inputQueryFilter{
		IndexName:  "aggregateTraintuple~worker~status",
		Attributes: worker + ", todo",
	}
	args = [][]byte{[]byte("queryFilter"), assetToJSON(filter)}
	resp = mockStub.MockInvoke("42", args)
	assert.EqualValuesf(t, 200, resp.Status, "when querying aggregate traintuple of worker with todo status - status %d and message %s", resp.Status, resp.Message)
	err = json.Unmarshal(resp.Payload, &queryTraintuples)
	assert.NoError(t, err, "aggregate traintuples should unmarshal without problem")
	assert.Exactly(t, out, queryTraintuples[0])
	...
	// Query AggregateTraintuple From key
	args = [][]byte{[]byte("queryAggregateTraintuple"), keyToJSON(aggregateTraintupleKey)}
	resp = mockStub.MockInvoke("42", args)
	assert.EqualValuesf(t, 200, resp.Status, "when querying aggregate traintuple with status %d and message %s", resp.Status, resp.Message)
	endTraintuple := outputAggregateTraintuple{}
	...
	assert.Exactly(t, expected, endTraintuple, "retreived AggregateTraintuple does not correspond to what is expected")
	...
	// assert.NotNil(t, payload.AggregateTraintuple, "when querying model tuples, payload should contain one traintuple")
...
func TestQueryTraintupleNotFoundAggregate(t *testing.T) {
	...
	// queryAggregateTraintuple: normal queryAggregateTraintuple
	args = [][]byte{[]byte("queryAggregateTraintuple"), keyToJSON(_key.Key)}
	resp = mockStub.MockInvoke("42", args)
	assert.EqualValuesf(t, 200, resp.Status, "when querying the aggregate traintuple - status %d and message %s", resp.Status, resp.Message)

	// queryAggregateTraintuple: key does not exist
	notFoundKey := "eedbb7c31f62244c0f34461cc168804227115793d01c270021fe3f7935482eed"
	args = [][]byte{[]byte("queryAggregateTraintuple"), keyToJSON(notFoundKey)}
	resp = mockStub.MockInvoke("42", args)
	assert.EqualValuesf(t, 404, resp.Status, "when querying the aggregate traintuple - status %d and message %s", resp.Status, resp.Message)

	// queryAggregateTraintuple: key does not exist and use existing other asset type key
	args = [][]byte{[]byte("queryAggregateTraintuple"), keyToJSON(algoHash)}
	resp = mockStub.MockInvoke("42", args)
	assert.EqualValuesf(t, 404, resp.Status, "when querying the aggregate traintuple - status %d and message %s", resp.Status, resp.Message)
}
...
func TestInsertTraintupleTwiceAggregate(t *testing.T) {
	...
	// create a aggregate traintuple and start a ComplutePlan
	inpTraintuple := inputAggregateTraintuple{
		Rank: "0",
	}
	inpTraintuple.createDefault()
	resp = mockStub.MockInvoke("42", methodAndAssetToByte("createAggregateTraintuple", inpTraintuple))
	assert.EqualValues(t, http.StatusOK, resp.Status)
	var _key struct{ Key string }
	json.Unmarshal(resp.Payload, &_key)

	// create a second aggregate traintuple in the same ComputePlan
	...
	// re-insert the same aggregate traintuple and expect a conflict error
	resp = mockStub.MockInvoke("42", methodAndAssetToByte("createAggregateTraintuple", inpTraintuple))
...
func TestAggregateTraintuplePermissions(t *testing.T) {
	scc := new(SubstraChaincode)
	mockStub := NewMockStubWithRegisterNode("substra", scc)
	registerItem(t, *mockStub, "trainDataset")
...
func TestAggregateTraintupleLogSuccessFail(t *testing.T) {
	for _, status := range []string{StatusDone, StatusFailed} {
		t.Run("TestAggregateTraintupleLog"+status, func(t *testing.T) {
			scc := new(SubstraChaincode)
			mockStub := NewMockStubWithRegisterNode("substra", scc)
			resp, _ := registerItem(t, *mockStub, "aggregateTraintuple")
			var _key struct{ Key string }
			json.Unmarshal(resp.Payload, &_key)
			key := _key.Key
			...
			// fetch back
			args := [][]byte{[]byte("queryAggregateTraintuple"), keyToJSON(key)}
			resp = mockStub.MockInvoke("42", args)
...
func TestQueryAggregateTraintuple(t *testing.T) {
	scc := new(SubstraChaincode)
	mockStub := NewMockStubWithRegisterNode("substra", scc)
	resp, inp := registerItem(t, *mockStub, "aggregateTraintuple")
	inpTraintuple := inp.(inputAggregateTraintuple)
	var keyOnly struct{ Key string }
	json.Unmarshal(resp.Payload, &keyOnly)

	args := [][]byte{[]byte("queryAggregateTraintuple"), keyToJSON(keyOnly.Key)}
	resp = mockStub.MockInvoke("42", args)
	assert.EqualValues(t, 200, resp.Status, "It should find the aggregate traintuple: %s", resp.Message)
	traintuple := outputAggregateTraintuple{}
	json.Unmarshal(resp.Payload, &traintuple)

	assert.Equal(t, aggregateTraintupleKey, traintuple.Key)
	assert.NotEmpty(t, traintuple.Worker)
	assert.Equal(t, inpTraintuple.Worker, traintuple.Worker)
	assert.Equal(t, worker, traintuple.Creator)
	assert.Equal(t, inpTraintuple.Tag, traintuple.Tag)
	assert.Equal(t, aggregateAlgoName, traintuple.Algo.Name)
	assert.Equal(t, inpTraintuple.AlgoKey, traintuple.Algo.Hash)
	assert.Equal(t, aggregateAlgoStorageAddress, traintuple.Algo.StorageAddress)
	assert.Equal(t, StatusTodo, traintuple.Status)
...
// TODO (aggregate): test ouput inModels
